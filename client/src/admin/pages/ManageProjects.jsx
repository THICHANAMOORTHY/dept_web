...
const ManageProjects = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentItem) await api.put(`/admin/projects/${currentItem._id}`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
      else await api.post('/admin/projects', data, { headers: { 'Content-Type': 'multipart/form-data' } });
      fetchProjects(); setIsModalOpen(false);
    } catch (err) { console.error(err); }
  };

  const columns = [
    { header: 'Image', accessor: 'imageUrl', render: (row) => <img src={row.imageUrl ? `http://localhost:5000${row.imageUrl}` : 'https://via.placeholder.com/40'} alt="project" style={{ width: '60px', height: '40px', objectFit: 'cover', borderRadius: '4px' }} /> },
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <h2 style={{ margin: 0 }}>Manage Student Projects</h2>
      ...
      <DataTable columns={columns} data={projects} onEdit={handleOpenModal} onDelete={(item) => { setCurrentItem(item); setDeleteModalOpen(true); }} />
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '400px', backgroundColor: 'white', boxShadow: '-4px 0 15px rgba(0,0,0,0.1)', zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '1.5rem', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between' }}>
            <h3 style={{ margin: 0 }}>{currentItem ? 'Edit Project' : 'Add Project'}</h3>
          ...
          <div style={{ padding: '1.5rem', flex: 1, overflowY: 'auto' }}>
            <form id="project-form" onSubmit={handleSubmit}>
              ...
              <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Project Title" required style={{ width: '100%', padding: '0.75rem', marginBottom: '1rem' }} />
              ...
              <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Project Description" style={{ width: '100%', padding: '0.75rem', marginBottom: '1rem', minHeight: '100px' }} />
      ...
      <ConfirmDeleteModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={async () => { await api.delete(`/admin/projects/${currentItem._id}`); fetchProjects(); setDeleteModalOpen(false); }} itemName={currentItem?.title} />
    </div>
  );
};
